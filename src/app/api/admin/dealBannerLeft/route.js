...
export async function POST(req) {
  try {
    ...
    let exitBanner = null;
    if(id){ 
      exitBanner = await DealBanner.findById(id);
    }

    if(photo && typeof photo  !== "string"){
      const imageExtension = path.extname(photo.name);
      const acceptExtensions = ['.jpg', '.png', 'jpeg', 'webp'];
      const imageName = `dealBannerLeft${rand(1111,9999)}${imageExtension}`
      if (!acceptExtensions.includes(imageExtension)) {
        ...
        await uploadImageFun(photo, uploadingPath, imageName, 887)
    ...
    if (id) {
      const banner = await DealBanner.findById(id);
      if (!banner) {
        return new Response(
      ...
      return new Response(
        JSON.stringify({ success: true, message: 'Left Banner updated successfully' }),
        { status: 200 }
      );
    } else {
...
export async function GET(req) {
  try {
    ...
    const banners = await DealBanner.find({pid:1}).sort({
      createdAt: -1
    });
    ...
    console.error('Error fetching left banners:', error);
    return new Response(
      JSON.stringify({ success: false, message: 'Error fetching left banners', error: error.message }),
      { status: 500 }
    );
  }
}
