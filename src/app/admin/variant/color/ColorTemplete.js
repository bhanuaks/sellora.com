...
function ColorTemplete() {
  ...
  const validate = () => {
    ...
    if (!formData.colorMap.trim()) newErrors.colorMap = 'Color Map is required.';
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    formDataToSubmit.append('colorMap', formData.colorMap);
    formDataToSubmit.append('status', formData.status);

    const url = '/admin-login/api/variant/color';
    const method = 'POST';
    ...
    try {
      ...
      if (response.ok) {
        ...
        $('.loader-container').css('display', 'none')
        setMessage({ type: 'error', text: result.message || 'Failed to save color.' });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      $('.loader-container').css('display', 'none')
      const response = await fetch(`/admin-login/api/variant/color`, {
        method: 'DELETE',
        headers: {
      ...
      if (response.ok) {
        ...
        $('.loader-container').css('display', 'none')
        setMessage({ type: 'error', text: result.message || 'Failed to delete color.' });
        toast.error(result.message);
      }
    } catch (error) {
      $('.loader-container').css('display', 'none')
          ...
          <div className="row">
            <div className="col-12">
              <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 className="mb-sm-0 font-size-18">Add Color</h4>
              </div>
            </div>
          </div>
          {/* Add Size Form */}
                        ...
                        <div className="mb-3">
                          ...
                          <div
                                style={{
                                  width: "100px",
                                  height: "50px",
                                  backgroundColor: formData.colorMap,
                            ...
                            <tr key={index}>
                              ...
                              <td><div
                                style={{
                                  width: "100px",
                                  height: "50px",
                                  backgroundColor: value.colorMap,
                                  border: "1px solid #ccc",
                                  marginTop: "10px",
                                }}
                              ></div></td>
