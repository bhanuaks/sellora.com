...
function ItemLengthTemplete() {
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.name.trim()) newErrors.name = 'Item Length Name is required.';
  ...
  const handleSubmit = async (e) => {
    ...
    const url = '/admin-login/api/variant/item-length';
    const method = 'POST';
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: result.message || 'Failed to save Item Length.' });
        toast.error(result.message);
      }
    } catch (error) {
      console.error('Error saving Item Length:', error);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      $('.loader-container').css('display', 'flex')
      const response = await fetch(`/admin-login/api/variant/item-length`, {
        method: 'DELETE',
        headers: {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: result.message || 'Failed to delete Item Length.' });
        toast.error(result.message);
      }
    } catch (error) {
      console.error('Error deleting Item Length:', error);
      setMessage({ type: 'error', text: 'An unexpected error occurred.' });
      toast.error(`Error: ${error.message}`);
    }
  };
  ...
  const fetchVariant = async () => {
    try {
      ...
      alert('Failed to fetch Item Length.');
    }
  };
          ...
          <div className="row">
            <div className="col-12">
              <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 className="mb-sm-0 font-size-18">Add Item Length</h4>
              </div>
            </div>
          </div>
          {/* Add Size Form */}
                  ...
                  <form onSubmit={handleSubmit} encType="multypart/form-data">
                    <div className="row">
                      <div className="col-lg-4">
                        <div className="mb-3">
                          <label className="form-label">Item Length (in cm)</label>
                          <input
                            ...
                            placeholder="Enter Item Length Name"
                          />
                          {errors.name && (
...
export default ItemLengthTemplete;
