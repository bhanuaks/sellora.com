...
function SizeTemplete() {
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.name.trim()) newErrors.name = 'Size Name is required.';
  ...
  const handleSubmit = async (e) => {
    ...
    const url = '/admin-login/api/variant/size';
    const method = 'POST';
    ...
    try {
      ...
      if (response.ok) {
        ...
        $('.loader-container').css('display', 'none')
        setMessage({ type: 'error', text: result.message || 'Failed to save size.' });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      $('.loader-container').css('display', 'flex')
      const response = await fetch(`/admin-login/api/variant/size`, {
        method: 'DELETE',
        headers: {
      ...
      if (response.ok) {
        ...
        $('.loader-container').css('display', 'none')
        setMessage({ type: 'error', text: result.message || 'Failed to delete size.' });
        toast.error(result.message);
      }
    } catch (error) {
          ...
          <div className="row">
            <div className="col-12">
              <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 className="mb-sm-0 font-size-18">Add Size</h4>
              </div>
            </div>
          </div>
          {/* Add Size Form */}
